    ...
    Column(modifier = modifier.padding(start = 10.dp, end = 10.dp, top = 15.dp)) {
        ...
        GoogleMap(
            ...
            for (locationWithTags in locations.value) {
                // Added this key as Claude Opus 4.6 extended and Gemini 3.1 Pro
                // explained that it is recommended with compose state in loops to avoid confusing compose
                key(locationWithTags.location.id) {
                    val locationPos = LatLng(
                        locationWithTags.location.latitude,
                        locationWithTags.location.longitude
                    )
                    val locationMarkerState = rememberUpdatedMarkerState(position = locationPos)

                    // This custom marker window was generated by Gemini 3.1 Pro as the
                    // default window cut off the description text
                    MarkerInfoWindowContent(
                        ...
                        // The 'marker' provided here allows you to access the title and snippet
                        Column(
                            modifier = Modifier
                                .widthIn(max = 250.dp) // Constrain width so it doesn't run off-screen
                                .padding(4.dp)
                        ) {
                            Text(
                                text = marker.title ?: "",
